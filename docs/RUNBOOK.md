...
> 中文注释：以下章节来自 main 侧 v3 的晨间工作流（morning 命令在合并后的代码中仍然可用）。
...
> 中文注释：以下章节来自 PR #7（私有执行器装载机制，executor-loader）。

## 附二. 私有执行器：公开仓不内置任何真实执行器

执行器涉及本机软件路径，属于私有配置。`npm run runtime -- daemon` 启动时会从私有目录装载执行器。
...
这来自一次真实事故：上一版曾报告"VS Code 已直接打开，进程已确认在运行"，交叉核查发现那台机器上根本没装 VS Code。所以现在的规则是：
**没有执行证据 → 如实标 `partial`，绝不标 `completed`。**
