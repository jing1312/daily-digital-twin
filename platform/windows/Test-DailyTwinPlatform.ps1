...
try {
    ...
    Write-DailyTwinCheck -Name '空白显式值返回 null，不得走兜底目录' -Condition ($null -eq (Resolve-DailyTwinPwsh -Preferred ' '))
    $nativeNonPwsh = if ($onWindows) { Join-Path $env:SystemRoot 'System32\cmd.exe' } else { '/bin/true' }
    Write-DailyTwinCheck -Name '完整路径的非 pwsh 程序返回 null' -Condition ($null -eq (Resolve-DailyTwinPwsh -Preferred $nativeNonPwsh))
    $wildcardPwsh = Join-Path $workRoot '*\pwsh.exe'
    Write-DailyTwinCheck -Name '通配符路径返回 null，不得搜索替代品' -Condition ($null -eq (Resolve-DailyTwinPwsh -Preferred $wildcardPwsh))

    $originalProgramFiles = $env:ProgramFiles
    $fakeProgramFiles = Join-Path $workRoot 'program-files'
    $fakeFallback = Join-Path $fakeProgramFiles 'PowerShell\7\pwsh.exe'
    New-Item -ItemType Directory -Force -Path (Split-Path -Parent $fakeFallback) | Out-Null
    [System.IO.File]::WriteAllText($fakeFallback, 'not an executable')
    try {
        $env:ProgramFiles = $fakeProgramFiles
        $requestedMissing = Join-Path $workRoot 'requested\pwsh.exe'
        Write-DailyTwinCheck -Name '点名不存在的 pwsh 路径时不得换成兜底目录那份' `
            -Condition ($null -eq (Resolve-DailyTwinPwsh -Preferred $requestedMissing))
    } finally {
